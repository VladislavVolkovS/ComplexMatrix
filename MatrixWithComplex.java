...
public class MatrixWithComplex {
    ...
    public MatrixWithComplex multiply(MatrixWithComplex matrix){
        if ((this.Matrix.length != matrix.Matrix[0].length) && (this.Matrix[0].length != matrix.Matrix.length) && (this.Matrix.length>1&&this.Matrix[0].length>1) && (matrix.Matrix.length>1&&matrix.Matrix[0].length>1)){
        ...
        if (this.Matrix.length==1&&this.Matrix[0].length==1){
            MatrixWithComplex result = new MatrixWithComplex(matrix.Matrix.length, matrix.Matrix[0].length);
            result = matrix.multconst(this.Matrix[0][0]);
            return result;
        }
        if (matrix.Matrix.length==1&&matrix.Matrix[0].length==1){
            MatrixWithComplex result = new MatrixWithComplex(matrix.Matrix.length, matrix.Matrix[0].length);
            result = this.multconst(matrix.Matrix[0][0]);
            return result;
        }
